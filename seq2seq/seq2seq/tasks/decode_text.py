...
def _unk_replace(source_tokens,
  ...
  for token, scores in zip(predicted_tokens, attention_scores):
    if np.ndim(scores) > 1:
      scores = scores[0, :]
    # raise ValueError(str(np.shape(scores)))
  
    if token == "UNK":
      max_idx = np.shape(source_tokens)[0]
      # new_scores = scores[:max_idx]
      # raise ValueError(str(np.shape(scores)))
...
class DecodeText(InferenceTask):
  ...
  def before_run(self, _run_context):
    ...
    if "attention_scores" not in fetches and "beam_search_output.original_outputs.attention_scores" in self._predictions:
  ...
  def after_run(self, _run_context, run_values):
    ...
    for fetches in unbatch_dict(fetches_batch):
      ...
      # nr_beams = np.ndim(predicted_tokens)
      # raise ValueError(str(np.shape(predicted_tokens)))
